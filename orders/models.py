...
class Order(models.Model):
    ...
    def update_total(self):
        # fsum([123,12.123]) #answer:135.246 and it's float
...
def post_save_cart_total(sender, instance, created, *args, **kwargs):
    # when cart changes the order size, total will change too in order app
    if not created:
        cart_obj = instance
...
def post_save_order(sender, instance, created, *args, **kwargs):
    # when order is created in the first place
    if created:
...
# post_save = right after the model change
# pre_save = right before the model save in the database
